...
def calculate_bc(start_date, end_date, bought_monthly, remaining_days=0, num_monthly_packs=0, buy_pack_later=False, buy_date=None):
    ...
    # Step 1: Calculate BC from the existing monthly pack
    if bought_monthly:
        if remaining_days > 0:
        ...
        # Step 2: Calculate BC after the existing monthly pack ends
        ...
        if days_after_monthly_end > 0:
            ...
            # Calculate BC from future monthly packs
            if num_monthly_packs > 0:
                total_bc += num_monthly_packs * 3300  # 3000 BC + 300 BC initial
            # Calculate BC from daily and weekly missions
            total_bc += (30 * days_after_monthly_end) + (1000 * weeks_after)
    else:
        # Step 3: If user decides to buy the package later
...
if bought_monthly == "yes":
    ...
    num_monthly_packs = int(input(f"From the end of your monthly period to the target date, the period is {weeks} weeks and {days_remainder} days. How many monthly packs will you buy? "))
    total_bc = calculate_bc(start_date, end_date, bought_monthly=True, remaining_days=remaining_days, num_monthly_packs=num_monthly_packs)
else:
    ...
    if buy_pack_later == "yes":
        ...
        total_bc = calculate_bc(start_date, end_date, bought_monthly=False, buy_pack_later=True, buy_date=buy_date, num_monthly_packs=num_monthly_packs)
    else:
        total_bc = calculate_bc(start_date, end_date, bought_monthly=False)

print(f"\nTotal BC you will obtain from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')} is: {total_bc} BC")